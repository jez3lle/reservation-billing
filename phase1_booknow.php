...
<div class="top-space">
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        select: function(info) {
            if (isCheckIn) {
                ...
                check_out = null; // Reset check-out when changing check-in
                ...
                if (new Date(info.startStr) <= new Date(check_in)) {    
    ...
    $.ajax({
        ...
        success: function (response) {
            console.log("Availability Response:", response); // Debugging
            $("#availabilityResult").html(response);

            if (response.includes("Available")) {
                console.log("✅ Room is available! Redirecting...");

                // Auto-redirect to booking form
                var url = "bookingform.php?check_in=" + encodeURIComponent(check_in) + "&check_out=" + encodeURIComponent(check_out);
        ...
        error: function () {
            console.log("Error: Failed to check availability.");
            $("#availabilityResult").html("<p class='error'>Error checking availability.</p>");
        }
    });
});
             });
